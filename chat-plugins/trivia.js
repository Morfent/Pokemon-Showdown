...
// Subclassing is slow! On top of that, subclassing to define the same three
// methods every time does not make much sense. Instead, we'll define the
// methods used to answer questions, calculate points, and reward correct
// answerers by using a decorator for each mode.
...
// correctly.
function firstModeDecorator(Target) {
	Object.defineProperties(Target.prototype, {
		answerQuestion: {
			value: function answerQuestion(answer, user) {
		...
		// This is triggerend if no one answers correctly at all.
		tallyAnswers: {
			value: function tallyAnswers() {
				this.phase = INTERMISSION_PHASE;
...
function timerModeDecorator(Target) {
	Object.defineProperties(Target.prototype, {
		answerQuestion: {
			value: function answerQuestion(answer, user) {
				let player = this.players[user.userid];
		...
		calculatePoints: {
			value: function calculatePoints(diff, totalDiff) {
...
// better).
function numberModeDecorator(Target) {
	Object.defineProperties(Target.prototype, {
		answerQuestion: {
			value: function answerQuestion(answer, user) {
				let player = this.players[user.userid];
		...
		calculatePoints: {
			value: function calculatePoints(correctPlayers) {
...
const commands = {
	new: function (target, room, user) {
		...
		let DecoratedTrivia;
		if (mode === 'first') {
			DecoratedTrivia = firstModeDecorator(Trivia);
		} else if (mode === 'timer') {
			DecoratedTrivia = timerModeDecorator(Trivia);
		} else if (mode === 'number') {
			DecoratedTrivia = numberModeDecorator(Trivia);
		}

		room.game = new DecoratedTrivia(room, mode, category, length, questions);
	},
...
module.exports = {
	Trivia: Trivia,
	firstModeDecorator: firstModeDecorator,
	timerModeDecorator: timerModeDecorator,
	numberModeDecorator: numberModeDecorator,
	commands: {
		trivia: commands,
		ta: commands.answer,
		triviahelp: [
